...
Real gm0, r0, rho0, tem0, dslope, p0_over_r0, pslope, gamma_gas, nu_iso, alpha, dens0, l0, opacity;
Real dfloor, tceil;
Real Omega0;
} // namespace
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  tceil = pin->GetOrAddReal("hydro","tceil",1.0);
  tceil = tceil / tem0;
      ...
      for (int k = ks; k <= ke; ++k) {
        ...
        for (int j = js; j <= je; ++j) {
          ...
          for (int i = is; i <= ie; ++i) {
            ...
            Real sound_speed = gamma_gas * prim(IPR,k,j,i) / prim(IDN,k,j,i);
    ...
    for (int j=1; j<=ngh; ++j) {
      for (int i=is; i<=ie; ++i) {
        for (int ifr=0; ifr<nfreq; ++ifr) {
          for(int n=0; n<nang; ++n){
            int ang = ifr*nang + n;
            Real& miuy=prad->mu(1,k,js-j,i,n);
            if (miuy < 0.0)
    ...
    for (int j=1; j<=ngh; ++j) {
      for (int i=is; i<=ie; ++i) {
        for (int ifr=0; ifr<nfreq; ++ifr) {
          for(int n=0; n<nang; ++n){
            int ang = ifr*nang + n;
            Real& miuy=prad->mu(1,k,je+j,i,n);
            if (miuy > 0.0)
