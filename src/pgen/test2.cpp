  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    ...
    for (int j = pmb->js; j <= pmb->je; ++j) {
      ...
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        d = sqrt(pow(r*cos(phi) - rp*cos(phip),2) + pow(r*sin(phi) - rp*sin(phip),2));
        Real dens = prim(IDN,k,j,i);
        Real velocity_x = prim(IVX,k,j,i);
        Real velocity_y = prim(IVY,k,j,i);
        epsilon = 0.3;
        R_H = rp*cbrt(gm_planet/(3*gm0));
        Real Fg = -(dens)* ((gm_planet*d) / (sqrt(pow(pow(d,2) + pow(epsilon,2)*pow(R_H,2), 3))));
        Real x_term = (r*cos(phi) - rp*cos(phip)) / d;
        Real y_term = (r*sin(phi) - rp*sin(phip)) / d;
        Real Fg_x = Fg*x_term;
        Real Fg_y = Fg*y_term;
